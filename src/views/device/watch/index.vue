  ...
  <div class="app-container">
    ...
    <div class="head-container">
      ...
      <el-table ref="table" v-loading="crud.loading" :data="crud.data" size="small" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
        ...
        <el-table-column v-permission="['admin','dWatch:edit','dWatch:del']" label="操作" width="150px" align="center">
          <template slot-scope="scope">
            <udOperation
              :data="scope.row"
...
<script>
import crudDWatch from '@/api/dWatch'
import {syncWatchConfigInfo} from '@/api/dWatch'
...
// crud交由presenter持有
const defaultCrud = CRUD({ title: '智能手环', url: 'api/dWatch', sort: 'id,desc', crudMethod: { ...crudDWatch }})
...
export default {
  ...
  data() {
    return {
      ...
      permission: {
        add: ['admin', 'dWatch:add'],
        edit: ['admin', 'dWatch:edit'],
        del: ['admin', 'dWatch:del']
      },
      rules: {
        id: [
  ...
  methods: {
    ...
    syncWatchConfigInfo(row){

      var data = {};
      data['id'] = row.id
      data['imei'] = row.imei

      syncWatchConfigInfo(data).then(res => {
        this.$notify({
          title: '同步成功',
          type: 'success',
          duration: 2500
